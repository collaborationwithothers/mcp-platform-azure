...
change ship with many supporting files, and reading those supporting diffs
teaches editing, not architecture. This session converts one merged PR into one
...
Find the core files: the one to three files that carry the concept. PRs
opened after the template gained a "Reading order" section declare them; for
older PRs, derive them yourself and say which files you chose and why.

## Walk through (the bulk of the session)

Work from the core files outward, Socratically. The goal is that Hari can
...
2. Walk the core files. For each, explain what it does in the running
...
3. If Hari's answer reveals a gap, rebuild that part of the explanation from
   ...
   the session short.
4. Summarise the supporting files in at most one line per file: which rule
   pulled each in. Do not walk supporting diffs unless Hari asks.
...
- Mechanism: how the core change works, two sentences max.

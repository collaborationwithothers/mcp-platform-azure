...
## Deliver the results in chat only

Return exactly one chat response, following the AGENTS.md output format:
verdict, numbered findings by severity, and "For Hari to check by hand". Chat
is the sole review deliverable. Do not publish an Artifact, render a page, or
write HTML to the session scratchpad.

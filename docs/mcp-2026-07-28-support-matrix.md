...
| Standard HTTP headers: Mcp-Method, Mcp-Name, Mcp-Param-{name} | The JSON-RPC method and target name are mirrored into HTTP headers so gateways can route and authorize without parsing bodies. Verified 2026-08-13 against the Streamable HTTP transport page (not just the changelog, whose "custom headers from tool parameters via `x-mcp-header`" summary line reads as if `x-mcp-header` were itself the wire header): `Mcp-Method` and `Mcp-Name` are the two Standard Request Headers; separately, a server MAY annotate a tool-parameter schema property with an `x-mcp-header` extension property, and "the `x-mcp-header` property specifies the name portion used to construct the header name `Mcp-Param-{name}`" - so `x-mcp-header` is the schema-side annotation and `Mcp-Param-{name}` is the resulting wire header; both names are correct, naming two different things, not competing claims. | Yes (required when negotiated version is 2026-07-28) | Spike - do the headers reach APIM policy context unmodified? | Adopt. Replaces the platform's earlier body-inspection authorization approach: issue #18 ("Per-tool authorization and tool blocking at the gateway", closed), confirmed 2026-08-12 by reading the issue. Security note: headers are client-supplied; policy must enforce header-body consistency (fail closed on mismatch) unless the SDK is confirmed to reject mismatches server-side [verify] |
...
| subscriptions/listen stream (SEP-2575) | Change notifications (tools/prompts/resources list changes, resource updates) arrive on a single opt-in stream, tagged with a subscription id; the old GET notification stream is gone. | Partial in 2.0.0 (re-verified 2026-08-13 at the source level, tightened from an earlier revision that leaned on issue-body prose rather than shipped code). At the `v2.0.0` git tag, `tests/ModelContextProtocol.Tests/Server/SubscriptionsListenTests.cs` exists and end-to-end tests SEP-2575 `subscriptions/listen` list-changed delivery (subscription acknowledgement tagged with a subscription id, `tools/list_changed` delivered only to a client that opted in, a `prompts/list_changed` change withheld from a client that did not) - the built-in dispatch for standard `*/list_changed` notifications ships in 2.0.0. At that same tag, `src/ModelContextProtocol.Core/Server/McpServerHandlers.cs` has no `SubscriptionsListenHandler` member: the public typed handler that lets server authors deliver custom subscription kinds is absent. At the `v2.1.0` tag, `McpServerHandlers.SubscriptionsListenHandler` and `McpServerBuilderExtensions.WithSubscriptionsListenHandler` both exist, added by PR modelcontextprotocol/csharp-sdk#1775 (merged 2026-08-04, closing issue #1662, released in v2.1.0 on 2026-08-05). This platform's tool set does not currently need custom subscription kinds, so the 2.0.0 built-in path is sufficient if adopted at 2.0.0; re-check the pinned SDK version against 2.1.0+ if custom subscriptions become a requirement. | Spike - long-lived stream through APIM: timeouts, buffering, policy interaction | Spike-first |
...
| Completion (completion/complete) | Argument autocompletion for prompts and resource templates. Confirmed live in 2026-07-28 (verified 2026-08-12): the spec's own `completion/complete` page is published under the `/specification/2026-07-28/` path with the new `resultType: "complete"` envelope in its example responses, so the feature is neither removed nor deprecated in this revision. | Yes. `completion/complete` predates 2026-07-28 and the 2026-07-28 changelog does not list it as removed or deprecated. `resultType: "complete"` is not completion-specific: per the changelog's major change 8, every ordinary result across the protocol now carries `resultType: "complete"` (versus `"input_required"` for an MRTR interim result), and `completion/complete`'s response is one instance of that general envelope, not a special case. The SDK v2.0.0 release adds "the `complete` result discriminator" (csharp-sdk#1684), which is the SDK-side implementation of that general `resultType` field | Spike | Spike-first, low priority |

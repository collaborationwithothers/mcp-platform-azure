...
- Documented: a `SessionId` exists as a first-class concept - it is exposed on the
  tool / resource / prompt invocation context the extension hands to your code.
...
| `X-MS-CLIENT-PRINCIPAL` | injected at the Function | Easy Auth V2 (strips any client-supplied copy, injects its own decoded claims) | `IdentityModeResolver` / authorization code (claims-based, no in-code signature check) | VERIFIED (ADR-006 "header trust chain"; docs/security.md) |
| `X-MS-TOKEN-AAD-ACCESS-TOKEN` | injected at the Function | Easy Auth token store (when enabled) | the tool, as the preferred inbound-token source (falls back to `Authorization`) | VERIFIED as the documented mechanism; requires the token store explicitly enabled (COMPATIBILITY.md) |
...
| `Mcp-Session-Id` | client <-> Function (per MCP spec) | MCP SDK / server (spec) | MCP SDK / server (spec) | SPEC-GENERIC: the wire header is MCP-spec / SDK behaviour; NOT documented as the Functions extension's own contract (azure-docs-verifier 2026-08-04). The server-side `SessionId` on the invocation context IS verified. |

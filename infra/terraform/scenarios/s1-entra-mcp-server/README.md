...
| `shared_observability_core_remote_state` | object | `{ storage_account_name, container_name, key }` for the persistent core state. The composition reads its Log Analytics workspace ID through OIDC-authenticated remote state for both Function App diagnostic settings. |

...
# child's registry): apply-time category discovery, allLogs if the target
...
# ContainerRegistryRepositoryEvents/ContainerRegistryLoginEvents and
# registry metrics are both supported diagnostic settings targets for
# Microsoft.ContainerRegistry/registries; verified at issue start (see
# COMPATIBILITY.md).
locals {
  diagnostic_target_resource_ids = {
    container_registry = module.registry.resource_id
  }
}

...
| Microsoft.ApiCenter/deletedServices (list, delete) | 2024-06-01-preview | infra/terraform/modules/api-center-registry/main.tf (data.azapi_resource_list.deleted_api_center_services, azapi_resource_action.purge_deleted_api_center) | API Center has genuine soft-delete: deleting the service (even via `terraform destroy`, or by deleting its resource group) leaves its name reserved by a tombstone until purge. A fixed-name live-test re-run hit 400 "name already taken" against a prior run's tombstone. `DeletedServices_ListBySubscription` and `DeletedServices_Delete` ("Permanently deletes specified service") both confirmed directly in the pinned-version OpenAPI spec pulled from Azure/azure-rest-api-specs (not just Learn's narrative docs), and via azapi_resource_action's documented `method = "DELETE"` support. Supersedes an earlier attempt using `properties.restore = true` unconditionally, which was disproven live: it 400s with "the service does not exist" when no tombstone is present, so no static restore value works in both states. This module now looks up and purges any existing tombstone before every create instead | 2026-07-14 | https://learn.microsoft.com/rest/api/resource-manager/apicenter/deleted-services/list |

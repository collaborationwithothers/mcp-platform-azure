...
| Microsoft.Identity.Web.Certificateless | 4.13.2 | src/McpTools/McpTools.csproj | ManagedIdentityClientAssertion(managedIdentityClientId: null) selects the system-assigned managed identity and signs the client assertion via api://AzureADTokenExchange. The installed package's actual public surface (confirmed two ways: reflecting the assembly directly, and a local `dotnet build` compile error/fix cycle -- not the Learn sample, which shows an older/simplified `GetSignedAssertion(CancellationToken)` shape that does not exist on 4.13.2): the inherited `ClientAssertionProviderBase.GetSignedAssertionAsync(AssertionRequestOptions)` returns `Task<string>` directly (the signed JWT), which is exactly the delegate shape `ConfidentialClientApplicationBuilder.WithClientAssertion(Func<AssertionRequestOptions,Task<string>>)` expects. | 2026-07-18 (azure-docs-verifier + reflection against the installed 4.13.2 assembly + local build) | https://learn.microsoft.com/dotnet/api/microsoft.identity.web.managedidentityclientassertion |
...
| Azure Functions MCP extension McpToolTrigger: inbound bearer token access (the OBO "inbound-token gap") | REFUTED (not merely undemonstrated) that McpToolTrigger exposes the caller's inbound HTTP headers/bearer token; UNVERIFIABLE whether FunctionContext.BindingContext.BindingData carries it through | src/McpTools/Tools/GetOrderStatus.cs (doc comment), docs/decisions/ADR-006 ("OBO exchange: the inbound-token gap") | Three independent findings, not one: (1) ToolInvocationContext (the extension's own source, not just its docs) exposes only Name/Arguments/SessionId/Transport. (2) A Functions method may declare exactly one trigger attribute, so McpToolTrigger cannot be paired with HttpTrigger on the same function to reach HttpRequestData. (3) ConfigureFunctionsWebApplication's ASP.NET Core integration explicitly does NOT expose the ASP.NET Core middleware pipeline or routing to the app, and its GetHttpRequestDataAsync() accessor is documented as scoped to [HttpTrigger]-dispatched invocations only. No Learn page or Microsoft-owned sample confirms or denies BindingContext.BindingData carrying headers for an MCP-triggered invocation specifically -- that piece stays UNVERIFIABLE, not built on. Blocks wiring OBO into GetOrderStatus.Run's live path in this ticket; see ADR-006's Growth paths for how to resolve it (a live instrumented trace, or a hosting-model redesign onto the public-preview self-hosted/custom-handler MCP surface). | 2026-07-18 (azure-docs-verifier, three-pass) | https://learn.microsoft.com/azure/azure-functions/functions-bindings-mcp-tool-trigger |

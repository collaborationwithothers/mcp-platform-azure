...
| Microsoft.ApiCenter/services soft-delete and properties.restore | EXPERIMENTAL/UNVERIFIED: restore = true set unconditionally on create | infra/terraform/modules/api-center-registry/main.tf (azapi_resource.api_center) | Verified: API Center has genuine soft-delete (Microsoft.ApiCenter/deletedServices REST reference: tombstones carry softDeletionDate/scheduledPurgeDate); a live gate re-run using a fixed service name hit 400 "name already taken" against a prior run's tombstone even though that run's terraform destroy had succeeded. Verified: properties.restore is documented to restore a soft-deleted instance ("if set to true all other properties will be ignored", ARM template reference, 2024-03-15-preview onward). NOT verified: its behavior on a genuinely first-ever create with nothing to restore; no purge REST operation for API Center's deleted services could be confirmed to exist, so purge-before-create is not an available alternative. Re-verify both cases at the next live-test run | 2026-07-14 (soft-delete verified; restore-on-fresh-create unverified) | https://learn.microsoft.com/rest/api/resource-manager/apicenter/deleted-services/list |

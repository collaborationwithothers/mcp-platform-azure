...
| Microsoft.ApiCenter/deletedServices (delete/purge) | 2024-06-01-preview | infra/terraform/modules/api-center-registry/main.tf (azapi_resource_action.purge_deleted_api_center) | API Center has genuine soft-delete: deleting the service (even via `terraform destroy`, or by deleting its resource group) leaves its name reserved by a tombstone until purge. A fixed-name live-test re-run hit 400 "name already taken" against a prior run's tombstone. `DeletedServices_Delete` ("Permanently deletes specified service") is confirmed directly in the pinned-version OpenAPI spec pulled from Azure/azure-rest-api-specs (not just Learn's narrative docs), and azapi_resource_action documents `method = "DELETE"` + `ignore_not_found`. The individual `deletedServices` resource is RESOURCE-GROUP scoped with a plain (non-location-qualified) name segment: `.../resourceGroups/{rg}/providers/Microsoft.ApiCenter/deletedServices/{name}` (verified 2026-07-14 against the 2024-06-01-preview spec and .NET SDK reference; only `ListBySubscription` is subscription-scoped without an RG, and there is NO `locations/` segment, unlike Microsoft.ApiManagement soft-delete). Because that id is fully derivable from static inputs, the module purges it directly with `ignore_not_found = true` (404 no-op when no tombstone) before every create. Supersedes two disproven approaches: (a) `properties.restore = true` unconditionally, which 400s with "the service does not exist" when no tombstone is present; and (b) gating the purge's `count` on a `data.azapi_resource_list` output, which failed at plan with "Invalid count argument" because a resource count cannot depend on data resolved only after apply. | 2026-07-14 | https://learn.microsoft.com/rest/api/resource-manager/apicenter/deleted-services/delete |

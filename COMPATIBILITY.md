...
| API Center data-plane registry token audience/resource | `https://azure-apicenter.net` (OAuth2 scope `.../user_impersonation`) | scripts/gate/invoke-and-assert.ps1 (registry poll) | The authenticated registry poll targets the data-plane audience `https://azure-apicenter.net` (`az account get-access-token --resource https://azure-apicenter.net`), the OIDC principal holding Azure API Center Data Reader. The servers-list path form carries a known Microsoft-doc inconsistency (the REST reference and the register-discover template include `/workspaces/{workspace}/`; the register-discover page's own example omits it). The module emits the `/workspaces/default/v0.1/servers` form; the gate tries it first, falls back to the stripped form, and records which one served the list. Audience/scope verified 2026-07-15 via azure-docs-verifier; the live path form is to be confirmed at the first live gate (not yet run) and recorded then. Re-check trigger: a docs correction, or a newer data-plane api-version. | 2026-07-15 | https://learn.microsoft.com/rest/api/dataplane/apicenter/apis/list |
